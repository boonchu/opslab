...
sub main {
    ...
    else
    {
        ...
        if ($res->is_success)
        {
            ...
            {
                ++$totalNodes;
		print "executor name : $computer->{'displayName'}, offline : $computer->{'offline'}, status : $computer->{'offlineCauseReason'}\n" if defined $verbose;
		...
		print "offline Node names $offlineNodeNames\n" if defined $offlineNodeNames;
    ...
    print $retStr if defined $retStr;
    exit $exitCode;
}
